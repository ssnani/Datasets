...
class LibriSpeechUttDataset(Dataset):
	...
	def __init__(self, path, T, size=None, return_vad=False, readers_range=None):
		self.file_list = []
		self.corpus  = self._exploreCorpus(path, 'flac', self.file_list)
	...
	def __getitem__(self, idx):
		...
		s_clean, vad_out = self._cleanSilences(utterance, 3, return_vad=True)
...
if __name__=="__main__":
	...
	T = 4
	dataset = LibriSpeechUttDataset(path, T, return_vad=True, size=1)
